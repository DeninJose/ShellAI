...
def constrain(listo):
    coords=makeCoords(listo)
#    print(coords)
#    print(len(listo)/2)

    for i in range(0,len(coords)):
        for j in range (i+1, len(coords)):
            if dist(coords[i],coords[j]) < 400:
                return 1
    return 0
...
def makeCoords(listo):
    list2=[]
    for i in range(0,len(listo)//2):
        list2.append((listo[2*i],listo[2*i+1]))
    return list2

def run(turb_coords):
    weight = constrain(turb_coords)
    turb_coords = makeCoords(turb_coords)
...
#    print(run((50,)*100))
    ...
    coors = makeList(turb_coords)
    bounds = [(50,3950)]*100
   
    
    #optimising
    
    res = differential_evolution(run, bounds) 
    print(res.x)